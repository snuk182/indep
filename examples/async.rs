...
#[macro_use]
extern crate log;
//`async` mod contains DI set for multi-threaded environments (uses Arc<RwLock<>> as an abstraction).
...
pub mod i1 {
    ...
    //The `Dependency` itself is an trait that DI pool accepts. Internally it is always implemented by a struct `ArcRwLockBox`,
    //which is a wrapper for Arc<RwLock<Box<Impl1>>>. This wrapper is required for Rust typecasting.
    indep_default_new_async!{Impl1}
}

//Sample implementation struct #2 - implements trait #2 (and trait Base). Depends on `Trait1` - see the mention of `indep_reqs_async` below.
pub mod i2 {
    use super::{Dependency,Dependent,Implementation};
    ...
    struct Impl2 {
    	//It is an Indep library requirement to have injectable dependencies in a form of Option<Arc<RwLock<Trait>>>
    ...
    //`Impl2` requires `Trait1` inside as member named `t1`. `t1` should have type Option<Arc<RwLock<Trait1>>>.
